...
from common.ma_request_schema import (
    UserAddrProdShipCreatePartialMaValidation,
    UserAddrProdShipCreateFullMaValidation
)
from common.pydantic_request_schema import (
    UserAddrProdShipCreateFullPydanticValidation,
    UserAddrProdShipCreatePartialPydanticValidation
)
...
class UserAddrProdShipPartialPydanticValidationResource(Resource):
    def post(self):
        data = request.get_json()

        try:
            validationResult = UserAddrProdShipCreatePartialPydanticValidation.model_validate(
                data, strict=False)
...
class UserAddrProdShipFullPydanticValidationResource(Resource):
    def post(self):
        data = request.get_json()

        try:
            validationResult = UserAddrProdShipCreateFullPydanticValidation.model_validate(
                data, strict=False)
        except PydanticValidationError as err:
            handle_pydantic_validation_err(err)
